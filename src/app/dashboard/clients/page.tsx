  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!submitData.full_name || !submitData.phone) {
        alert('Le nom complet et le téléphone sont obligatoires');
        return;
      }
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <label className="text-sm font-semibold text-gray-600 block mb-1">Téléphone</label>
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <p className="text-lg font-medium text-gray-900">{viewingClient.email || 'Non renseigné'}</p>
                </div>

                <div className="bg-gray-50 p-4 rounded-lg">
                  <label className="text-sm font-semibold text-gray-600 block mb-1">Adresse</label>
                  <p className="text-lg font-medium text-gray-900">{viewingClient.address || 'Non renseignée'}</p>
                </div>

                <div className="bg-gray-50 p-4 rounded-lg">
                  <label className="text-sm font-semibold text-gray-600 block mb-1">Carte d'identité</label>
                  <p className="text-lg font-medium text-gray-900">{viewingClient.national_id || 'Non renseigné'}</p>
                </div>
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  <label className="text-sm font-semibold text-gray-600 block mb-2">Informations Système</label>\n                  <div className="space-y-1 text-sm text-gray-600">
                    <p>Branche: <span className="font-medium text-gray-900">{viewingClient.branch_name || 'Non spécifié'}</span></p>
                    <p>Créé le: <span className="font-medium text-gray-900">{new Date(viewingClient.created_at).toLocaleDateString('fr-FR')}</span></p>
                  </div>
                </div>
              </div>
            ...
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    Téléphone: {selectedCustomerForDocs.phone}
                  </p>
                </div>
                <button
                  onClick={() => {
              ...
              {loadingDocs ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600 text-lg">Aucun dossier de crédit trouvé</p>
                ...
                <div>
                  <div className="bg-gradient-to-r from-purple-50 to-blue-50 p-4 rounded-lg mb-6">
                    <p className="text-sm font-semibold text-gray-700">
                      {customerFiles.length} dossier{customerFiles.length > 1 ? 's' : ''} trouvé{customerFiles.length > 1 ? 's' : ''} • {customerDocuments.length} document{customerDocuments.length > 1 ? 's' : ''}
                    </p>
                  </div>
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-3 mb-2">
                                ...
                                <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                                  ...
                                  {file.status === 'accepted' ? 'Accepté' :
                                   file.status === 'rejected' ? 'Rejeté' :
                                   file.status === 'completed' ? 'Terminé' : 'En révision'}
                                </span>
                              </div>
                                    ...
                                    >
                                      ...
                                      <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-gray-500 mt-1">
                                          {isImage ? '🖼️ Image' : isPDF ? '📄 PDF' : '📎 Fichier'}
                                        </p>
                                      </div>
